...
const bucket = (hour: number, plays: number): ClockBucket => ({ hour, plays });
...
describe('buildHourClock', () => {
  ...
  it('accumulates plays into one slice per hour', () => {
    const clock = buildHourClock([bucket(9, 2), bucket(9, 3), bucket(22, 5)]);
    expect(clock.hours).toHaveLength(24);
  ...
  it('sets fraction relative to the busiest hour and reports the peak', () => {
    const clock = buildHourClock([bucket(8, 1), bucket(20, 4)]);
  ...
  it('orders slices 0..23', () => {
    const clock = buildHourClock([bucket(0, 1)]);
