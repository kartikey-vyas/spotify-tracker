<script lang="ts">
  ...
  onMount(async () => {
    try {
      rows = await getRankings({ entityType: 'track', start: range.start, end: range.end, metric: 'minutes', limit: 100 });
    } catch (caught) {
...
<section class="page">
  ...
  {:else}
    <section class="grid cols-2">
      ...
      <div class="panel"><RankingTable {rows} entityType="track" metric="minutes" /></div>
    </section>
  {/if}
</section>
