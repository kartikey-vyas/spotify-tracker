...
export function scoreMusicBrainzRecordings(
  input: { name: string; durationMs: number | null; artistNames: string[] },
  recordings: MusicBrainzRecording[]
  ...
  const scored = recordings.map((recording): ScoredMusicBrainzRecording => {
    ...
    const durationDeltaMs = input.durationMs !== null && durationMs !== null
      ? Math.abs(input.durationMs - durationMs)
      : null;
    let score = 0;
  ...
  scored.sort((left, right) => {
    ...
    return (left.durationDeltaMs ?? Number.POSITIVE_INFINITY) -
      (right.durationDeltaMs ?? Number.POSITIVE_INFINITY);
  });
  if (scored.length > 1 && scored[0].score - scored[1].score <= 3) scored[0].ambiguous = true;
  return scored;
}

export async function lookupMusicBrainzByIsrc(
  isrc: string,
  options: { userAgent: string; fetchImpl?: typeof fetch }
