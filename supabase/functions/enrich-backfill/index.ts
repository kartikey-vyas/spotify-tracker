 ...
 * A rejection is evicted rather than cached: otherwise one transient artist
 * upsert failure would be replayed to every later track sharing that artist,
 * failing them all without a retry.
 */
...
Deno.serve(async (req: Request) => {
  try {
    ...
    // Spotify's long Retry-After is an app-wide cap, not a per-request burst.
    // Persist its deadline across stateless Edge invocations so the 15-minute
    // cron records a clean skip instead of sending three doomed pool requests.
    const { data: latestRateCap, error: cooldownError } = await supabase
      .from('enrichment_runs')
      .select('aborted,started_at,abort_retry_after_seconds')
      .eq('aborted', true)
      .gt('failed', 0)
      .not('abort_retry_after_seconds', 'is', null)
      .order('started_at', { ascending: false })
      .limit(1)
      .maybeSingle();
    if (cooldownError) {
      throw new Error(`Loading Spotify enrichment cooldown failed: ${cooldownError.message}`);
    }
    ...
    // Keyed "<user_id> <local_date>" — the same composite the queue's
    // primary key uses. Repointing is global, so one track can dirty dates
    // belonging to several users.
    ...
    await pool(tracks, concurrency, async (track) => {
      ...
      try {
        ...
        // Not scoped to a user: `tracks.last_refreshed_at` above is global, so
        // any user's events left unrepointed here would never get another
        // chance — the track would read as enriched forever.
        ...
        for (const row of (updatedEvents ?? []) as Array<{
          user_id: string | null;
          local_date: string;
        }>) {
          // Legacy archived rows carry a null user_id and have no rollups.
          if (row.user_id) affectedDates.add(`${row.user_id} ${row.local_date}`);
        }
    ...
    if (affectedDates.size > 0) {
      const { error: queueError } = await supabase.from('rollup_refresh_queue').upsert(
        [...affectedDates].map((key) => {
          const [user_id, local_date] = key.split(' ');
