-- Scope the listening clock to the last 30 days.
--
-- The clock now shows recent rhythm rather than lifetime habits, so its query
-- gains a `local_date >= melbourne_today - 29` filter (matching the last_30_days
-- window used elsewhere). Everything else in build_overview_payload is unchanged
-- from 20260630120000_overview_clock_release_calendar.sql.
