-- Overview viz: full-history calendar, listening clock, release-year histogram.
--
-- Three additions to build_overview_payload, all consumed by the Overview page:
--   * calendar.daily      — replaces calendar.last_365_days with the FULL daily
--                           history so the calendar can show a fixed Jan–Dec grid
--                           per year and cycle between years.
--   * clock               — all-time plays bucketed by LOCAL weekday × hour, for a
--                           24×7 listening-clock heatmap. Local time is derived from
--                           played_at via the Australia/Melbourne zone (matching how
--                           local_date is computed elsewhere).
--   * release_years       — plays by album release year, excluding compilations
--                           (whose release_date is the compilation's, not the song's)
--                           and any album whose release_date isn't a plausible year.
--
-- Everything else is unchanged from 20260623220801_overview_rank_by_plays.sql.
...
begin
  return jsonb_build_object(
    ...
    'clock', coalesce((
      select jsonb_agg(to_jsonb(r) order by r.dow, r.hour)
      from (
        select
          extract(dow  from (played_at at time zone 'Australia/Melbourne'))::int as dow,
