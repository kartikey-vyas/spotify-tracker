...
1. Owner runs `pnpm invite <email>`; Supabase (custom SMTP via Resend on `krtky.dev`) emails an invite link. Accounts are only ever created server-side, so hosted signups stay disabled — invite-only holds.
2. Invitee clicks the link, lands signed-in at `/app/` with no profile yet, and sets a password + display name/slug/visibility. The client calls `auth.updateUser({ password })` then the `complete-onboarding` Edge Function (authenticated; the session is the gate — no invite code).
